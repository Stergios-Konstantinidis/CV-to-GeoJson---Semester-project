...
public class MeshGeneratiorStergios : MonoBehaviour
{
    /// <summary>
    ...
    public sealed class ARPlaneMeshVisualizer : MonoBehaviour
    {
        ...
        void OnBoundaryChanged(ARPlaneBoundaryChangedEventArgs eventArgs)
        {
            ...
            if (lineRenderer != null)
            {
                ...
                List<Vector3> pointsToUse = new List<Vector3>();
                for (int i = 0; i < boundary.Length; ++i)
                {
                    var point2 = boundary[i];
                    lineRenderer.SetPosition(i, new Vector3(point2.x, 0, point2.y));
                    pointsToUse.Add(new Vector3(point2.x, 0, point2.y));
                }
                Debug.Log("should send now");
                GeojsonGenerator jsonGenerator = new GeojsonGenerator(pointsToUse);
                byte[] bytes = System.Text.Encoding.UTF8.GetBytes(jsonGenerator.GetGeoJson());
                UnityWebRequest www = UnityWebRequest.Put("https://webhook.site/f49e79b9-81f0-4168-ba01-28d3b7d00201", bytes);
